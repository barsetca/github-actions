...
- **GET /time/convert** — конвертация времени из UTC в местное время города; город задаётся названием на русском или английском (параметры: `city`, опционально `utc_time`)
...
## Пример GET /time/convert

Город задаётся названием на русском или английском. По умолчанию используется текущее время UTC.

Текущее время в Екатеринбурге:
```
GET /time/convert?city=Екатеринбург
```

Время 15:00 UTC в Екатеринбурге (получится 20:00):
```
GET /time/convert?city=Екатеринбург&utc_time=15:00
```
(или с полной датой: `utc_time=2025-02-04%2015:00:00`)

Пример ответа:
```json
{
  "utc_time": "2025-02-04T15:00:00+00:00",
  "city": "Екатеринбург",
  "timezone": "Asia/Yekaterinburg",
  "converted_time": "2025-02-04T20:00:00+05:00",
  "converted_time_local": "2025-02-04 20:00:00 +05"
}
```
